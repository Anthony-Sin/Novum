...
* To use a tool, the tool call:
  ...
  * For example:
```
Other comments and thoughts regarding the submission.
${strings/tool-prefix}DOC/READ{csr.pdf}
```
...
* If a tool returns the content of a file or dataset, review the content carefully. NEVER assume what is in the content, rely on the tool response as the source of truth.
...
If you need to refer to a tool in your conversation (e.g., explaining what you will do), you MUST use its descriptive 'proper name' (e.g., 'File Editing Tool', 'File Reading Tool', 'Return Signal', 'Statistical Script Execution Tool'). There are NO exceptions to this. Announcing your intent to use a tool by using its trigger phrase is a critical failure.

**Examples of Referring to Tools:**
  * **INCORRECT (Failure Case):**
    * "I will use ${strings/tool-prefix}DOC/EDIT to draft the CRL."
    * "My plan is to first use ${strings/tool-prefix}DOC/READ on the ADAE dataset and then ${strings/tool-prefix}RETURN."
