...
export class Orchestrator {
  ...
  private investigationHalted?: boolean;
  ...
  constructor(
    ...
    this.signal?.addEventListener('abort', () => {
      ...
      this.sendMessage(JSON.stringify({
        ...
        current_status_message: '# Investigation shutdown initiated',
      }));
    });
    ...
    this.toolContext = {
      ...
      investigationHalted: this.investigationHalted,
    };
  }
  ...
  private async generateProjectSummary(): Promise<{result: string; retrospective: string; feedback: string}> {
    ...
    this.updateLog(`Generating Investigation Summary.`)
  ...
  private async summarizeOrchestratorUpdate(lastOrchestratorResponse: string): Promise<string> {
    let current_status_message = "Analyzing the forensic evidence."
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      await this.updateProgressLog(`Initiating Scientific Integrity Overseer with ${OVERSEER_FREQUENCY_MINS} minute frequency.`)
      ...
      await this.updateProgressLog(`You can ask questions about the investigation progress using the Chat window. When finished, I will suggest follow-up forensic actions.`);
      ...
      await this.updateProgressLog('\n## Enriched Research Integrity Audit Request\n----');
      ...
      await this.updateLog(`# Enriched Research Integrity Request:\n${this.initialPrompt}`);
      ...
      while (!isDone) {
        if (this.signal?.aborted) {
          ...
          this.sendMessage(JSON.stringify({
            status: 'PROGRESS_UPDATES',
            current_status_message: 'Cancelling Investigation.',
            completed_status_message: '## Cancelling Investigation\n\nThe Orchestrator has received a cancellation request. Shutting down.'
          }));
        ...
        if (feedback) {
          ...
          switch (feedback.action) {
            case 'RESTART':
              ...
              this.sendMessage(JSON.stringify({
                status: 'PROGRESS_UPDATES',
                completed_status_message: `\n### Overseer\nThe Overseer has ordered a restart. Resetting state and restarting the investigation.`,
              }));
              ...
              const restartGuidance = `The investigation has been RESTARTED by the Overseer. You MUST follow this new guidance:\n${feedback.guidance}`;
            ...
            case 'ABANDON':
              this.updateLog('The overseer has decided to abandon the investigation.', false);
              this.sendMessage(JSON.stringify({
                status: 'PROGRESS_UPDATES',
                completed_status_message: '\n### Overseer\nThe Scientific Integrity Overseer has abandoned the investigation.',
              }));
              isDone = true; 
              continue; 
